...
<div id="page-wrapper">
                            ...
                            <div class="col-sm-3">
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control js-select2" name="active" id="active">
                                        ...
                                        <option value='1' {{ request()->input('active') == '1'? 'selected="selected"' :
                                            '' }}>開啟</option>
                                        <option value='0' {{ request()->input('active') == '0'? 'selected="selected"' :
                                            '' }}>關閉</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-sm-3 text-right">
                                <div class="col-sm-12"><button class="btn btn-warning"><i class="fa fa-search  "></i>
                                        查詢</button></div>
                            </div>
                        </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <button class="btn btn-info btn-sm toggle-show-model"
                                    data-toggle="modal" data-target="#row_detail" @click="show({{$val->id}})"><i class="fa fa-search"></i>
                                    </button>                                    
                                    {{-- @endif --}}
                                    {{-- @if ($share_role_auth['auth_update'] && $v['status_code'] == 'DRAFTED' &&
                                    $v['created_by'] == $data['user_id']) --}}
                                    <a class="btn btn-info btn-sm"
                                        href="{{route('web_category_products.edit',$val->id)}}">修改</a>
                                    {{-- @endif --}}

                                    {{-- @if ($share_role_auth['auth_delete'] && $v['status_code'] == 'DRAFTED' &&
                                    $v[created_by'] == $data['user_id']) --}}
    ...
    @include('Backend.WebCategoryProducts.list_detail')
</div>
@endsection
@section('js')
<script>
