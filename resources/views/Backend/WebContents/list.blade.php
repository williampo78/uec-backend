                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-heading">
                        <form role="form" id="select-form" method="GET" action="" enctype="multipart/form-data">
                            <div class="row">
                                <div class="col-sm-1 text-right"><h5>類別</h5></div>
                                <div class="col-sm-3">
                                    <select name="code" id="code">
                                        <option value="">請選擇</option>
                                        @foreach($data['category'] as $cate)
                                            <option value="{{$cate['code']}}" {{ (isset($data['getData']['code']) && $data['getData']['code'] == $cate['code'])? 'selected':''  }}>{{$cate['description']}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-sm-1 text-right"><h5>問題描述</h5></div>
                                <div class="col-sm-3"><input class="form-control" name="content_name" id="content_name" placeholder="模糊查詢"
                                                             value="{{ $data['getData']['content_name']?? '' }}"></div>
                                <div class="col-sm-1 text-right"><h5>類型</h5></div>
                                <div class="col-sm-3">
                                    <select name="target" id="target">
                                        <option value="">請選擇</option>
                                        @foreach($data['target'] as $k=>$v)
                                            <option value="{{$k}}" {{ (isset($data['getData']['target']) && $data['getData']['target'] == $k)? 'selected':''  }}>{{$v}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            @if($share_role_auth['auth_create'])
                                <div class="col-sm-2">
                                    <a class="btn btn-block btn-warning btn-sm" id="btn-new"
                                       href="{{route('webcontents.create')}}"><i class="fa fa-plus"></i> 新增</a>
                                </div>
                            @endif
                        </div>
                        <hr>
